...
class ProductBuyer{
	...
	Product[] items = new Product[10]; // 구입한 상품들을 저장하는 일종의보관서 (창고 or 카트)
	...
	public void refund(Product p) {
		...
		for(int i=0; i<items.length; i++) {
			if(p.getpNumber()==items[i].getpNumber()) {
				//i번째에 해당하는 상품을 뒤에 삼품이 계속 덮어쓴다.(제거하는것)
	...
	public void summary() {
		/*
		 * for 문을 돌면서 구입한 물건의 총합과 총 목록을 출력
		 */
		int sum = 0;
		String itemList = " ";
